...
class BitslothEfficientGRPO(torch.autograd.Function):
    ...
    def forward(
        ...
        def compute_loss(
            ...
            loss, completion_length, mean_kl, delta, flat_is_ratio, coef_1, _mask = (
                grpo_compute_loss(
                    ref_logps,
                    new_logps,
                    old_logps,
    ...
    **kwargs,
    ...
    if trainer.args.bitsloth_grpo_mini_batch is None:
        if not hasattr(trainer, "_has_autotuned"):
            ...
            B, multiplier = autotune_batch_and_chunks(
                ...
                vocab_dim,
                dtype_bytes,
                trainer.args.bitsloth_logit_chunk_multiplier,
            )
    ...
    class bitsloth_Offloaded_Log_Softmax(torch.autograd.Function):
        ...
        def forward(
            ...
            ctx.saved_hidden_states = (
                hidden_states.detach().contiguous().to("cpu", non_blocking=True)
            )
            ...
            if pixel_values is None:
                ...
                if new_hidden_states_chunk.shape[-1] == lm_head.shape[1]:
                    ...
                    logprobs_chunk = chunked_selective_log_softmax(
                        new_hidden_states_chunk, completion_ids, temperature
                    )
