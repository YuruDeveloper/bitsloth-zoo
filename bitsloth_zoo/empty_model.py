...
def is_comparable(val):
    ...
    return isinstance(
        val, (int, float, bool, str, list, tuple, type(None), torch.dtype, Enum)
    )
...
def compare_dicts(orig_dict, new_dict, prefix=""):
    ...
    for key in sorted(all_keys):
        ...
        if isinstance(orig_val, dict) and isinstance(new_val, dict):
            ...
            if orig_val != new_val:
                print(
                    f"Dict key {key_path} mismatch: original {orig_val} != new model {new_val}"
                )
        elif type(orig_val) != type(new_val):
            print(
                f"Dict key {key_path} type mismatch: original {type(orig_val)} != new model {type(new_val)}"
            )
...
def compare_attributes(original_model, new_model):
    ...
    config_keys = (
        _extract_all_config_keys(original_model.config)
    ...
    for (name, module), (orig_name, original_module) in zip(
        ...
        buffer_names = {
            name for name, _ in original_module.named_buffers(recurse=False)
        }
            ...
            if type(original_val) != type(new_val):
                if original_comparable or new_comparable:
                    type_mismatches.append(
                        f"{name}.{attr}: original {type(original_val).__name__} != new {type(new_val).__name__}"
                    )
                continue
                ...
                if isinstance(original_val, dict) and isinstance(new_val, dict):
                    ...
                    if original_val != new_val:
                        value_mismatches.append(
                            f"{name}.{attr}: original {original_val} != new {new_val}"
                        )
            except Exception as e:
                type_mismatches.append(f"{name}.{attr}: comparison failed - {str(e)}")

            try:
                if isinstance(original_val, PretrainedConfig) and isinstance(
                    new_val, PretrainedConfig
                ):
                    compare_dicts(
                        original_val.to_dict(),
                        new_val.to_dict(),
                        prefix=f"{name}.{attr}",
                    )
            except Exception as e:
...
def copy_attributes(original_model, new_model):
    ...
    for (name, module), (_, original_module) in zip(
        new_model.named_modules(), original_model.named_modules()
    ):
        buffer_names = [
            name for name, _ in original_module.named_buffers(recurse=False)
        ]
    ...
    if os.environ.get("BITSLOTH_ENABLE_LOGGING", "0") == "1":
        print(
            f"✅ Copied {copied_count} attributes (including {dict_copied_count} config-related dicts)"
        )
        if dict_skipped_count > 0:
            print(f"📋 Skipped {dict_skipped_count} non-config dictionaries")
        if skipped_count > 0:
            print(
                f"⏭️ Skipped {skipped_count} total attributes (tensors, modules, non-config dicts, etc.)"
            )
            if skipped_count <= 10:
                print(f"    Skipped: {skipped_attrs}")
            else:
                print(
                    f"    Sample: {skipped_attrs[:5]}... and {skipped_count - 5} more"
                )
    ...
    with init_empty_weights(include_buffers=False):
        if model_name is not None:
            try:
                ...
                original_meta_model = AutoModelForCausalLM.from_pretrained(
                    model_name, **kwargs
                )
            except Exception as e:
                error = str(e)
...
def create_empty_vision_model(config, dtype=torch.float16):
    ...
    try:
        ...
        print(
            f"Failed to create original_meta_model for {model_cls.__name__}. Error {e}"
        )
        import traceback
    ...
    vision_layers = getattr(config.vision_config, "num_hidden_layers", None) or getattr(
...
def create_empty_model(config, dtype=torch.float16, is_vision_model=False):
    ...
    if is_vision_model:
        ...
        new_model, original_meta_model, num_layers = create_empty_causal_lm(
            config, dtype
        )
...
def set_additional_modules(new_model, quant_state_dict, config):
    ...
    if pad_token_id is not None:
        assert pad_token_id < quant_state_dict[embed_tokens_key].shape[0], (
            f"Pad token id {pad_token_id} out of bounds for vocab size {quant_state_dict[embed_tokens_key].shape[0]}"
        )
    ...
    set_embedding(
        language_model.embed_tokens, embed_tokens_key, pad_token_id
    )  # This sets the embedding that we generally find in language (sub)model

    if "model.visual.pos_embed.weight" in quant_state_dict:
        # This is to handle visual embeddings in Qwen 3 VL
        set_embedding(
            new_model.model.visual.pos_embed,
    ...
    additional_keys = set(
        x
        for x in quant_state_dict.keys()
        if not any(
            substr in x
            for substr in (
                "layers",
...
def get_model_layer_config(return_non_layered=True):
    ...
    return {
        ...
        for key, value in layer_templates.items()
        if key != "non_layered_components" or return_non_layered
    }
...
def extract_vision_layers(vllm_internals, state_dict, quant_state_dict, get_state_dict):
    ...
    num_layers_to_iterate = (
        max(layer_counts.values()) if isinstance(layer_counts, dict) else layer_counts
    )
            ...
            if layer_module is not None:
                if "qkv" in layer_path:
                    if model_type in ("qwen2_5_vl", "qwen3_vl"):
                        ...
                        get_state_dict(
                            layer_path, 0, state_dict, layer_module, slice_weights=False
                        )
                    else:
                        get_state_dict(
                            f"{layer_path.replace('qkv_proj', 'q_proj')}",
                            0,
                            state_dict,
                            layer_module,
                        )
                        get_state_dict(
                            f"{layer_path.replace('qkv_proj', 'k_proj')}",
                            1,
                            state_dict,
                            layer_module,
                        )
                        get_state_dict(
                            f"{layer_path.replace('qkv_proj', 'v_proj')}",
                            2,
                            state_dict,
                            layer_module,
                        )
                    ...
                    get_state_dict(
                        f"{layer_path.replace('gate_up_proj', 'gate_proj')}",
                        0,
                        state_dict,
                        layer_module,
                    )
                    get_state_dict(
                        f"{layer_path.replace('gate_up_proj', 'up_proj')}",
                        1,
                        state_dict,
                        layer_module,
                    )
        ...
        if component is not None:
            if hasattr(component, "weight"):
                ...
                for param_name, param in component.named_parameters():
                    ...
                    if param_name.replace(".weight", "") in non_layered_components:
                        continue
    ...
    if component is not None:
        ...
        state_dict[f"{path}.weight"] = weight.reshape(
