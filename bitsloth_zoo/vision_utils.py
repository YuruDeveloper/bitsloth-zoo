    ...
    if isinstance(image, Image.Image):
        ...
        if "bytes" in image and image["bytes"]:
            ...
            image_obj = Image.open(
                requests.get(image["url"], stream=True, timeout=30).raw
            )
    ...
    for vision_info in vision_infos:
        if "image" in vision_info or "image_url" in vision_info:
            ...
            video_input, video_sample_fps = fetch_video(
                vision_info, image_factor=size_factor, return_video_sample_fps=True
            )
    ...
    def _resize_images_inplace(self, image):
        ...
        for i, img in enumerate(image):
            if type(image_size) is tuple:
                ...
                new_w = (w * image_size + self.size_func(img) // 2) // self.size_func(
                    img
                )
                new_h = (h * image_size + self.size_func(img) // 2) // self.size_func(
                    img
                )
