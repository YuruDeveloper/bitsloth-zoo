...
if TextInput != str:
    TYPE_MAPPINGS[TextInput] = t.Any
if PreTokenizedInput != List[str]:
    TYPE_MAPPINGS[PreTokenizedInput] = t.Any

if TransformersKwargs != t_TypedDictMeta:
    TYPE_MAPPINGS[TransformersKwargs] = t_TypedDictMeta
if FlashAttentionKwargs != t_TypedDictMeta:
    TYPE_MAPPINGS[FlashAttentionKwargs] = t_TypedDictMeta
if LossKwargs != t_TypedDictMeta:
    TYPE_MAPPINGS[LossKwargs] = t_TypedDictMeta
...
def canonicalize_annotation(annotation: Any) -> Any:
    ...
    if type(annotation) is tuple and len(annotation) == 2:
        ...
        # (typing.Unpack, (<class 'typing._TypedDictMeta'>,))
            ...
            annotation[0] == t_Unpack
            and type(annotation[1]) is tuple
            and len(annotation[1]) == 1
            and "Kwargs" in str(annotation[1][0])
        ):
            annotation = (
                t_Unpack,
                (t_TypedDictMeta,),
        ...
        # (typing.Unpack, <class 'typing._TypedDictMeta'>,)
            ...
            annotation[0] == t_Unpack
            and type(annotation[1]) is type
            and "Kwargs" in str(annotation[1])
        ):
            annotation = (
                t_Unpack,
                (t_TypedDictMeta,),
...
def get_function_fingerprint(func: Callable) -> List[Dict[str, Any]]:
    ...
    for kk, param in enumerate(signature_parameters):
        ...
        if "kwargs" in param_name.lower():
            ...
            if (
                (param_kind == VAR_KEYWORD_ID)
                and (annotation == EMPTY)
                and (len(signature_parameters) - 1 == kk)
            ):
                annotation = (
                    t_Unpack,
                    (t_TypedDictMeta,),
        ...
        if (
            (param_name == "x")
            and (len(signature_parameters) == 2)
...
def can_safely_patch(
    ...
    if len(old_fp) != len(new_fp):
        ...
        if removed_flags_list == (
            "output_attentions",
            "output_hidden_states",
        ):
            return (
                False,
                f"New function removed output_attentions and output_hidden_states",
            )
    ...
    for old_param, new_param in zip(old_fp, new_fp):
        if (old_param["name"], old_param["kind"]) != (
            new_param["name"],
            new_param["kind"],
        ):
        ...
        if new_param["is_required"] and not old_param["is_required"]:
            return (
                False,
                f"Parameter '{new_param['name']}' changed from optional to required",
            )
...
def list_stored_originals(target_obj: Any) -> List[str]:
    ...
    for attr_name in dir(target_obj):
        if attr_name.startswith("_original_") and not attr_name.startswith(
            "_original___"
        ):
