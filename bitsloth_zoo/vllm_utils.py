...
def _maybe_set_flashinfer_workspace_base():
    if os.environ.get("FLASHINFER_WORKSPACE_BASE"):
        return
    try:
        from .compiler import get_compile_folder, BITSLOTH_COMPILE_LOCATION
...
def _return_nothing(*args, **kwargs):
    return None
...
def _return_self_tokenizer(self, *args, **kwargs):
...
if importlib.util.find_spec("vllm") is not None:
    ...
    if not BITSLOTH_ENABLE_LOGGING:
    ...
    try:
        ...
        if hasattr(
            ...
            vllm.model_executor.model_loader.bitsandbytes_loader.dequantize_dq = (
                dequantize_dq
            )
        elif hasattr(
    ...
    if not hasattr(
        ...
        def _apply_4bit_weight(
            ...
            out = torch.empty(
                out_dim_0, out_dim_1, dtype=inference_dtype, device=x.device
            )
    ...
    def patch_vllm_lora_load_tensors():
        ...
        try:
            ...
            vllm.worker.model_runner.LRUCacheWorkerLoRAManager = (
                PatchedLRUCacheWorkerLoRAManager
            )
        except:
            pass
    ...
    def patch_vllm_set_inductor_config():
        try:
            ...
            vllm.compilation.compiler_interface.set_inductor_config = (
                set_inductor_config
            )
        except:
            pass
    ...
    def from_dict(cls, qs_dict: Dict[str, Any], device: torch.device) -> "QuantState":
        ...
        if "nested_absmax" in qs_dict:
            ...
            with torch.autocast(device_type="cuda", enabled=False):
                offset = torch.tensor(
                    qs_dict["nested_offset"], dtype=torch.float32, device="cuda"
                )
            state2 = cls(
...
def patch_vllm_enable_sleep_mode():
    ...
    def sleep(self, offload_tags: Optional[Union[Tuple[str, ...], str]] = None) -> None:
        ...
        logger.debug(
            f"Set of tags {set_of_tags} and len of data {len(self.pointer_to_data.items())}"
        )
        ...
        for ptr, data in self.pointer_to_data.items():
            ...
            if data.tag in offload_tags:
                ...
                cpu_offloads += 1
            logger.debug(
                f"data's tag is {data.tag} and is offloaded to cpu? {data.tag in offload_tags}"
            )
        ...
        logger.debug(
            f"CPU offloads {cpu_offloads} true offloads {true_offloads} total {total_offloads}"
        )
        gc.collect()
    ...
    def print_memory_summary(self):
        ...
        for data in self.pointer_to_data.values():
            ...
            if data.tag == "weights":
                ...
                kv_cache_count += 1
        logger.debug(
            f"Total weights memory: {weights_total / 1e9:.2f} GB for {weights_count} items"
        )
        logger.debug(
            f"Total KVCache memory: {kv_cache_total / 1e9:.2f} GB for {kv_cache_count} items"
        )
    ...
    def get_patched_generate(original_generate):
        def check_sleep_mode(self):
            ...
            return (
                hasattr(engine, "vllm_config")
                and hasattr(engine.vllm_config, "model_config")
                and getattr(engine.vllm_config.model_config, "enable_sleep_mode", False)
            )
...
def vllm_dynamic_quant_supported(
    ...
    for module in llm_int8_skip_modules:
        ...
        if find_regex.search(module) is None:
            return False
    return True
...
def _get_vllm_state_dict(
    ...
    try:
        ...
        needs_transpose_check = "layer.weight_scale.data.T.contiguous()" in getsource(
            maybe_post_process_fp8_weight_block
        )
    except Exception as e:
    ...
    if needs_transpose_check:
        ...
        try:
            from vllm.utils.deep_gemm import (
                is_deep_gemm_supported as vllm_is_deep_gemm_supported,
            )
        ...
        try:
            cutlass_block_fp8_supported = (
                torch.ops._C.cutlass_scaled_mm_supports_block_fp8(sm_cap)
            )
        except Exception as e:
            logger.info(
    ...
    def get_state_dict(
        prefix, kk, state_dict, proj, slice_weights=True, slice_index=-1
    ):
        ...
        if qweight.dtype == torch.float8_e4m3fn:
            ...
            if weight_scale.ndim == 2:
                if weight_scale.shape[1] > 1:
                    ...
                    is_compressed_linear = "CompressedTensors" in str(
                        type(getattr(proj, "quant_method", None))
                    )
                    if is_compressed_linear:
                        ...
                        should_use_deepgemm = (
                            ...
                            and getattr(proj, "orig_dtype", torch.bfloat16)
                            == torch.bfloat16
                            and qweight.shape[0] % 128 == 0
                            and qweight.shape[1] % 128 == 0
                        )
                        if (
                            sm_cap == 90
                ...
                if slice_weights:
                    weight_scale = weight_scale[
                        scale_offsets[kk] : scale_offsets[kk + 1]
                    ]
        ...
        if bias is not None:
            ...
            if shrink_size is not None and (
                "embed_tokens" in prefix or "lm_head" in prefix
            ):
    ...
    get_state_dict(
        f"{vllm_text_model_prefix}.embed_tokens",
        0,
        state_dict,
        embed_tokens,
        slice_weights=False,
    ...
    def _is_fused_module(name: str) -> bool:
        ...
        return (
            isinstance(packed, (list, tuple)) and len(packed) == 1 and packed[0] == name
        )
    ...
    for kk in range(len(vllm_text_model.layers)):
        ...
        if hasattr(layer, "self_attn"):
            ...
            if use_fused_qkv:
                ...
                get_state_dict(
                    f"{prefix}.qkv_proj", 0, state_dict, qkv_proj, slice_weights=False
                )
            else:
            ...
            name = re.sub(
                r"\.(\d+)\.",
                r"[\1].",
                prefix.replace("model.language_model", "language_model.model", 1)
                + ".qkv_proj",
            )
        ...
        if use_fused_gate_up:
            ...
            # see https://github.com/vllm-project/vllm/blob/9b693d023cf595e60b5346fdeeb41cf2a6eda838/vllm/model_executor/models/phi3.py
            get_state_dict(
                f"{vllm_text_model_prefix}.layers.{kk}.mlp.gate_up_proj",
                0,
                state_dict,
                proj,
                slice_weights=False,
            ...
            get_state_dict(
                f"{vllm_text_model_prefix}.layers.{kk}.mlp.gate_proj",
                0,
                state_dict,
                proj,
            )
            get_state_dict(
                f"{vllm_text_model_prefix}.layers.{kk}.mlp.up_proj", 1, state_dict, proj
            )

        proj = layer.mlp.down_proj
        get_state_dict(
            f"{vllm_text_model_prefix}.layers.{kk}.mlp.down_proj", 0, state_dict, proj
        )
        ...
        for layernorm_name in layernorm_names:
            vllm_name = layernorm_name.replace(f".{kk}.", f"[{kk}].").replace(
                vllm_text_model_prefix, "vllm_text_model"
            )
            try:
    ...
    if is_vision_model:
        ...
        extract_vision_layers(
            vllm_internals, state_dict, quant_state_dict, get_state_dict
        )
    # Norm
    ...
    if not getattr(text_config, "tie_word_embeddings", False):
        lm_layer = [
            mod for name, mod in vllm_internals.named_modules() if "lm_head" in name
        ]
        # Use get_state_dict for consistent extraction and automatic truncation
    ...
    for key in old_state_dict:
        try:
            ...
            torch.testing.assert_close(
                old_val, new_val, check_stride=False, atol=1e-4, rtol=1e-3
            )
            ...
            if key == "lm_head.weight":
                ...
                if key1 is not None and key2 is not None:
                    try:
                        torch.testing.assert_close(
                            old_state_dict[key1].contiguous(),
                            new_state_dict[key2].contiguous(),
                            check_stride=True,
                        )
                    except Exception:
                        failures[key] = error
...
def convert_vllm_to_huggingface(
    ...
    if quantization_config != {} or bnb_config is not None:
        ...
        if quantization_config != {}:
            if quant_method == "bitsandbytes":
                kwargs["compress_statistics"] = quantization_config[
                    "bnb_4bit_use_double_quant"
                ]
                kwargs["quant_type"] = quantization_config["bnb_4bit_quant_type"]
                kwargs["quant_storage"] = _get_dtype(
                    quantization_config["bnb_4bit_quant_storage"]
                )
                ...
                try:
                    from transformers.integrations.fbgemm_fp8 import (
                        FbgemmFp8Linear,
                ...
                block_size = (
                    weights.get("block_size", block_size) if weights else block_size
                )
    ...
    for kk in range(layer_count):
        for layer_name in layer_names:
            ...
            if fp8_weight_scale is not None:
                assert fp8_weight_scale.ndim in [1, 2], (
                    f"we only support row quantized (ndim=1) and block quantized(ndim=2) fp8 but found {fp8_weight_scale.ndim}"
                )

            if layer_name in quant_state_dict:
                # for attributes of type nn.Parameter, there's no .weight
                layer_name_br = re.sub(
                    r"\.([\d]{1,})\.", r"[\1].", layer_name.replace("model.", "", 1)
                )
                ...
                if fp8_weight_scale.ndim == 1:
                    ...
                    layer = FbgemmFp8Linear(
                        in_features=0, out_features=0, bias=has_bias, weight_dtype=dtype
                    ...
                    layer.weight_scale = torch.nn.Parameter(
                        fp8_weight_scale, requires_grad=False
                    )
                ...
                layer.weight = torch.nn.Parameter(
                    getattr(weight, "data", weight), requires_grad=False
                )
                layer.bias = bias
            else:
            ...
            layer_name = re.sub(
                r"\.([\d]{1,})", lambda x: f"[{x.group(1)}]", layer_name
            )
    ...
    )  # try using text config for VLMs
...
def approximate_vllm_memory_usage(
    ...
    weights = mlp_size * hd
    maximum_activation = activation_qkv + residual_memory + activation_mlp + weights
    # 2 bytes with 25% extra just in case
    maximum_activation = (maximum_activation * 1.25) * 2
    if not account_for_gradients:
    ...
    if load_in_4bit:
        ...
        factor = 8 / 5  # Very vague approximation. Will fix later
    bytes_for_model = (
    ...
    if cuda_graph_overhead:
        ...
        per_graph_estimate = (
            hd * n_layers * 4
        )  # bytes, rough per-graph activation estimate
        _cuda_graph_overhead = num_cuda_graphs * per_graph_estimate
        _cuda_graph_overhead = max(
            _cuda_graph_overhead, int(0.15 * 1024**3)
        )  # floor at 0.15 GiB
        _cuda_graph_overhead = min(
            _cuda_graph_overhead, int(1.0 * 1024**3)
        )  # cap at 1.0 GiB
...
def load_vllm(
    ...
    if Version(vllm_version) < Version("0.11.0"):
        ...
        if ten_percent >= 9.0:
            standby_target_gpu_util = 0.9
        elif ten_percent >= 4.0:
            standby_target_gpu_util = 0.875
        elif ten_percent >= 2.5:
            standby_target_gpu_util = 0.85
        elif ten_percent >= 2.0:
            standby_target_gpu_util = 0.825
        elif ten_percent >= 1.4:
            standby_target_gpu_util = 0.8
        elif ten_percent >= 1.0:
            standby_target_gpu_util = 0.75
        else:
            standby_target_gpu_util = 0.7
    else:
        ...
        if ten_percent >= 4.0:
            ...
            standby_target_gpu_util = 0.775
        else:
            standby_target_gpu_util = 0.75
        # Reduce memory usage for newer vLLM versions since it OOMs
        if BITSLOTH_ENABLE_LOGGING:
            logger.info(
                f"Decreasing VRAM further since vLLM version >= 0.11.0 uses more"
            )
        standby_target_gpu_util *= 0.95
        if BITSLOTH_ENABLE_LOGGING:
            logger.info(
                f"Further reduced standby_target_gpu_util = {standby_target_gpu_util:.4f}"
            )
    pass
    if BITSLOTH_ENABLE_LOGGING:
    ...
    assert not (use_bitsandbytes and is_fp8), (
        f"`load_in_4bit` and `load_in_8bit` should be set to false for loading FP8 quantized models with fast inference"
    )
    ...
    if (
        ...
        _has_nvcc = (
            ...
            or os.path.isfile(
                os.path.join(os.environ.get("CUDA_PATH", ""), "bin", "nvcc")
            )
        ...
        if not _has_nvcc or not _has_ninja:
            _missing = []
            if not _has_nvcc:
                _missing.append("nvcc (CUDA compiler)")
            if not _has_ninja:
            ...
            if (
                "VLLM_ATTENTION_BACKEND" in os.environ
                and os.environ["VLLM_ATTENTION_BACKEND"] == ""
            ):
                del os.environ["VLLM_ATTENTION_BACKEND"]
    ...
    if memory_left_for_kv_cache_gb <= 2:
        max_num_batched_tokens, approx_max_num_seqs = 2048, 8  # - 8
    elif memory_left_for_kv_cache_gb <= 4:
        max_num_batched_tokens, approx_max_num_seqs = 2048, 16  # - 16
    elif memory_left_for_kv_cache_gb <= 8:
        max_num_batched_tokens, approx_max_num_seqs = 4096, 32  # - 16
    elif memory_left_for_kv_cache_gb <= 12:
        max_num_batched_tokens, approx_max_num_seqs = 4096, 48  # - 16
    elif memory_left_for_kv_cache_gb <= 16:
        max_num_batched_tokens, approx_max_num_seqs = 6144, 64  # Default
    elif memory_left_for_kv_cache_gb <= 24:
        max_num_batched_tokens, approx_max_num_seqs = 6144, 80  # + 16
    elif memory_left_for_kv_cache_gb <= 40:
        max_num_batched_tokens, approx_max_num_seqs = 8192, 96  # + 16
    elif memory_left_for_kv_cache_gb <= 48:
        max_num_batched_tokens, approx_max_num_seqs = 8192, 112  # + 16
    elif memory_left_for_kv_cache_gb <= 80:
        max_num_batched_tokens, approx_max_num_seqs = 8192, 128  # + 16
    elif memory_left_for_kv_cache_gb > 80:
        max_num_batched_tokens, approx_max_num_seqs = 8192, 256  # + 16
    ...
    if not is_vision_model:
        if memory_left_for_kv_cache_gb <= 8:
            chunked_prefill_tokens = 1024  # + 0
        elif memory_left_for_kv_cache_gb <= 12:
            chunked_prefill_tokens = 1536  # + 512
        elif memory_left_for_kv_cache_gb <= 16:
            chunked_prefill_tokens = 2048  # + 512
        elif memory_left_for_kv_cache_gb <= 24:
            chunked_prefill_tokens = 3072  # + 1024
        elif memory_left_for_kv_cache_gb <= 40:
            chunked_prefill_tokens = 4096  # + 1024
        elif memory_left_for_kv_cache_gb <= 48:
            chunked_prefill_tokens = 4608  # + 512
        elif memory_left_for_kv_cache_gb <= 80:
            chunked_prefill_tokens = 8192  # + 4096
        else:
            chunked_prefill_tokens = 8192  # + 0
    ...
    if compilation_config == 3:
        try:
            ...
            if Version(vllm_version) > Version("0.11.0") or Version(
                torch_version
            ) > Version("2.9.0"):
    ...
    good_keys = inspect.signature(
        AsyncEngineArgs if use_async else EngineArgs
    ).parameters.keys()
            ...
            if trials >= 2 or bitsloth_vllm_standby:
                ...
                if bitsloth_vllm_standby and (
                    ...
                    raise MemoryError(
                        ...
                        f"  2. Disable standby mode: remove os.environ['BITSLOTH_VLLM_STANDBY'] = '1'\n"
...
def get_peft_config(save_directory):
    with open(
        os.path.join(save_directory, "adapter_config.json"), encoding="utf-8"
    ) as f:
        config = json.load(f)
    return config
...
def prepare_vllm_lora_loading(model):
    ...
    vllm_model = (
        model.vllm_engine.llm_engine.model_executor.driver_worker.model_runner.model
    )
    ...
    for v_layer, m_layer in zip(vllm_model.model.layers, model.model.model.layers):
        ...
        vllm_loras_B.append(
            (
                v_layer.self_attn.qkv_proj.lora_b_stacked[0],
                sq,
            )
        )
        vllm_loras_B.append(
            (
                v_layer.self_attn.qkv_proj.lora_b_stacked[1],
                sk,
            )
        )
        vllm_loras_B.append(
            (
                v_layer.self_attn.qkv_proj.lora_b_stacked[2],
                sv,
            )
        )
        ...
        vllm_loras_B.append(
            (
                v_layer.self_attn.o_proj.lora_b_stacked[0],
                so,
            )
        )
        ...
        vllm_loras_B.append(
            (
                v_layer.mlp.gate_up_proj.lora_b_stacked[0],
                sg,
            )
        )
        vllm_loras_B.append(
            (
                v_layer.mlp.gate_up_proj.lora_b_stacked[1],
                su,
            )
        )
        ...
        vllm_loras_B.append(
            (
                v_layer.mlp.down_proj.lora_b_stacked[0],
                sd,
            )
        )
    pass
    ...
    for model_lora_B, (
        vllm_lora_B,
        s,
    ) in zip(model_loras_B, vllm_loras_B):
...
def convert_lora_modules(
    ...
    if (hasattr(model, "peft_config") and "default" in model.peft_config) and (
        model.peft_config["default"].peft_type == PeftType.LORA
    ):
        state_dict = model.state_dict().items()
        state_dict = {
            k: v.detach().clone()
            for k, v in state_dict
            if (v.dtype != dtype) and (".lora_A.default" in k or ".lora_B.default" in k)
        }
        if len(state_dict) == 0:
            return {}
...
def return_lora_modules(
    ...
    if state_dict == {} or state_dict is None:
        return
    dtype = _get_dtype(dtype_from_config(model.config) if dtype is None else dtype)
...
def load_lora(model, save_directory, load_tensors=False, lora_request_id=None):
    ...
    if load_tensors:
        ...
        lora_request = LoRARequest(
            str(lora_request_id), lora_request_id, save_directory
        )
    pass
    # vllm_lora_already_loaded(model)
    ...
    if n_batches is None:
        if "bitsloth_VLLM_BATCHES" in os.environ:
            ...
            if total_memory_gb <= 8:
                n_batches = llm.approx_max_num_seqs // 10
            elif total_memory_gb <= 16:
                n_batches = llm.approx_max_num_seqs // 5
            elif total_memory_gb <= 24:
                n_batches = llm.approx_max_num_seqs // 2
            else:
...
def _test_same_model(model, new_model, input_ids):
    ...
    torch.testing.assert_close(
        model.model.embed_tokens.weight, new_model.model.embed_tokens.weight
    )
    ...
    for i, (old, new) in enumerate(zip(model.model.layers, new_model.model.layers)):
        ...
        torch.testing.assert_close(
            old.input_layernorm.weight, new.input_layernorm.weight
        )
        A = old.input_layernorm(A)
        B = new.input_layernorm(B)

        AA, _ = old.self_attn(
            A.clone(), attention_mask=None, position_embeddings=rotary_A
        )
        BB, _ = new.self_attn(
            B.clone(), attention_mask=None, position_embeddings=rotary_B
        )
        ...
        attentionA, _ = f(
            ...
            attention_mask=None,
            dropout=0.0
            if not old.self_attn.training
            else old.self_attn.attention_dropout,
        ...
        attentionB, _ = f(
            ...
            attention_mask=None,
            dropout=0.0
            if not new.self_attn.training
            else new.self_attn.attention_dropout,
        ...
        residualB = B
        torch.testing.assert_close(
            old.post_attention_layernorm.weight, new.post_attention_layernorm.weight
        )
    ...
    try:
        ...
        if hasattr(model, "lm_head") and hasattr(new_model, "lm_head"):
            if (
                model.lm_head.weight is not None
                and new_model.lm_head.weight is not None
            ):
                torch.testing.assert_close(
                    model.lm_head.weight, new_model.lm_head.weight
                )
...
def _test_is_same_vlm(model, new_model, processor, test_backward=False):
    ...
    messages = [
        {
            ...
            "content": [
                {
                    "type": "image",
                    "image": "https://files.worldwildlife.org/wwfcmsprod/images/Sloth_Sitting_iStock_3_12_2014/story_full_width/8l7pbjmj29_iStock_000011145477Large_mini__1_.jpg",
                },
    ...
    if test_backward:
        ...
        print(
            f"Backward gradient statistics match for {len(matches)} layers: {matches}"
        )
        print(
            f"Backward gradient statistics mismatch for {len(mismatches)} layers: {mismatches}"
        )
...
def _test_get_vllm_state_dict(
    ...
    new_model = convert_vllm_to_huggingface(
        quant_state_dict, config, dtype, is_vision_model=is_vision_model
    )
    ...
    if not is_vision_model:
        ...
        messages = [
            ...
            [
                {
                    "role": "user",
                    "content": "What is the capital of France? Describe it.",
                },
            ],
            [
            ...
            [
                {
                    "role": "user",
                    "content": "Derive the formula for an infinite sum of 1, 1/2, 1/4, 1/8 and so on.",
                },
            ],
        ] * counts
